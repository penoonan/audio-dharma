...
use AudioDharma\Base\TaxonomyInterface as Taxonomy;

class PostTypeInvalidArgumentException extends \InvalidArgumentException{}
class PostTypeInvalidLabelKeyException extends \InvalidArgumentException{}
class PostTypeInvalidRewriteKeyException extends \InvalidArgumentException{}
    ...
    private function getArgs()
    {
        ...
        foreach ($this->args as $k => $v) {
            if (!in_array($k, $this->arg_keys)) {
                Throw new PostTypeInvalidArgumentException('Specified argument key "'.$k . ' => ' . $v .'" specified in custom post type ' . get_class($this) . ' is not valid. Valid argument keys are: ' . print_r($this->arg_keys) . '.');
            } else {
                $args[$k] = $v;
            }
        }
    ...
    private function getLabels()
    {
        ...
        foreach ($this->labels as $k => $v) {
            if (in_array($k, $this->label_keys)) {
                ...
                Throw new PostTypeInvalidLabelKeyException('Specified label key "' . $k . '" in post type ' . get_class($this) . ' is not valid. Valid label keys are: ' . print_r($this->label_keys) .'.');
            }
        }
    ...
    private function getRewrite()
    {
        ...
        foreach ($this->rewrite as $k => $v) {
            if (in_array($k, $this->rewrite_keys)) {
                ...
                Throw new PostTypeInvalidRewriteKeyException('Specified rewrite key "' . $k . '" in post type ' . get_class($this) . ' is not valid. Valid rewrite keys are: ' . print_r($this->label_keys) .'.');
            }
        }
        return count($vals) > 0 ? $vals : null;
    }
